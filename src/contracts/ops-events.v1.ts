...
export interface BaseEventPayload {
  ...
  /**
   * Actor (usuario logueado que ejecuta la acción).
   * Debe venir siempre para auditoría y trazabilidad.
   */
...
export interface CashAdvancePayload extends BaseEventPayload {
  ...
  // Campos extra que ya estás usando en ShiftConsole
  reason?: string;
  reason_label?: string;
...
export const validateEvent = (payload: any): string | null => {
  ...
  // ✅ Actor obligatorio
  if (!payload.actor_user_id) return 'Falta actor_user_id (usuario)';
  // opcional: validar formato mínimo de email si viene
  if (payload.actor_email && typeof payload.actor_email === 'string') {
    const ok = payload.actor_email.includes('@');
    if (!ok) return 'actor_email inválido';
  }
